    ...
    V: Clone + Send + Sync + 'static,
{
    ...
    /// `Self::mapping`.
    values: ObservableVector<SendWrapperInsideWasm<V>>,
}
    ...
    V: Clone + Send + Sync + 'static,
{
    ...
    pub(crate) fn insert(&mut self, key: K, value: V) -> usize {
        match self.mapping.get(&key) {
            Some(position) => {
                self.values.set(*position, SendWrapperInsideWasm::new(value));
            ...
            None => {
                ...
                self.values.push_back(SendWrapperInsideWasm::new(value));
    ...
    where
        ...
        L: Hash + Eq + ?Sized,
    {
        self.mapping.get(key).and_then(|position| self.values.get(*position)).map(|x| &**x)
    }
    ...
    pub(crate) fn stream(
        ...
        Vector<SendWrapperInsideWasm<V>>,
        impl Stream<Item = Vec<VectorDiff<SendWrapperInsideWasm<V>>>>,
    ) {
        ...
        L: Hash + Eq + ?Sized,
    {
        ...
        Some(self.values.remove(position).take())
    }
}
